...
const openCreateModal = () => {
    ...
    formErrors.value = {};
    form.value = {
        patient_id: props.patients[0]?.patient_id
            ? String(props.patients[0].patient_id)
            : '',
...
const handleSaveLogbook = async (submitToDpjp: boolean) => {
    ...
    try {
        if (editingLogbook.value) {
            await axios.put(
                `/koas/logbook/${editingLogbook.value.clinical_logbook_id}`,
                form.value,
            );
        } else {
        ...
        if (err.response?.status === 422 && err.response?.data?.errors) {
            ...
            alert(
                err.response?.data?.message ||
                    'Gagal menyimpan logbook klinis.',
            );
        }
    } finally {
...
> = {
    ...
    submitted: {
        label: 'Menunggu Review DPJP',
        badge: 'bg-amber-50 text-amber-800 border-amber-300 animate-pulse',
        dot: 'bg-amber-500',
    },
    approved: {
        label: 'Disetujui DPJP',
...
<template>
    ...
    >
        ...
        <div class="mx-auto max-w-7xl space-y-6">
            ...
            >
                <div class="space-y-1">
                    ...
                    <p class="text-xs text-[#000000]/60 sm:text-sm">
                        Dokumentasikan riwayat kasus klinis, asistensi prosedur
                        medis, dan refleksi harian dengan verifikasi resmi DPJP.
                    </p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <motion.button
                        ...
                        class="inline-flex min-h-[44px] cursor-pointer items-center gap-2 rounded-xl bg-[#065f46] px-5 py-2.5 text-sm font-bold text-[#ffffff] shadow-sm transition-colors hover:bg-[#054d38]"
                    >
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                >
                    ...
                    <div
                        class="mt-1 font-mono text-2xl font-bold text-neutral-600"
                    >
                        {{ stats.draft }}
                    </div>
                </div>
            ...
            <div class="space-y-4" v-if="logbooks.data.length > 0">
                <motion.div
                    ...
                    :animate="{ opacity: 1, y: 0 }"
                    class="flex flex-col justify-between gap-4 rounded-2xl border border-[#000000]/10 bg-[#fffff3] p-6 shadow-sm transition-all hover:border-[#065f46]/30"
                >
                    <div
                    ...
                    >
                            ...
                            >
                                ...
                                <span
                                    ...
                                    <strong class="text-[#065f46]">{{
                                        item.doctor?.name || 'Dokter Spesialis'
                                    }}</strong></span
                                >
                                <span>•</span>
                                <span
                        ...
                        >
                            <motion.button
                                ...
                                class="inline-flex min-h-[44px] cursor-pointer items-center gap-1.5 rounded-lg border border-purple-200 bg-purple-50 px-3.5 py-2 text-xs font-bold text-purple-800 transition-colors hover:bg-purple-100"
                            >
                            ...
                            <motion.button
                                ...
                                class="inline-flex min-h-[44px] cursor-pointer items-center gap-1.5 rounded-lg bg-[#065f46] px-3.5 py-2 text-xs font-bold text-[#ffffff] transition-colors hover:bg-[#054d38]"
                            >
                            ...
                            <motion.button
                                ...
                                class="inline-flex min-h-[44px] cursor-pointer items-center gap-1.5 rounded-lg border border-[#000000]/15 bg-[#fffff3] px-3 py-2 text-xs font-medium text-[#000000] transition-colors hover:bg-[#edede2]"
                            >
            ...
            <div
                ...
                v-else
            >
                <BookOpen class="mx-auto mb-3 size-12 text-[#000000]/30" />
                <h3 class="font-serif text-lg font-bold text-[#000000]">
                    Belum Ada Kasus Klinis
                </h3>
                <p class="mx-auto mt-1 mb-4 max-w-md text-xs text-[#000000]/60">
                    Mulailah mencatat kasus anamnesis, pemeriksaan fisik, atau
                    asistensi tindakan medis selama rotasi stase Anda.
                </p>
                <button
                    type="button"
                    @click="openCreateModal"
                    class="inline-flex min-h-[44px] cursor-pointer items-center gap-2 rounded-xl bg-[#065f46] px-5 py-2 text-xs font-bold text-[#ffffff] shadow-sm hover:bg-[#054d38]"
                >
                    <Plus class="size-4 text-[#beedc0]" />
        ...
        >
            ...
            >
                        ...
                        <div>
                            ...
                            >
                                {{
                                    editingLogbook
                                        ? 'Edit Logbook Klinis'
                                        : 'Catat Kasus Klinis Baru'
                                }}
                            </h2>
                            <p class="text-xs text-[#000000]/60">
                                Isi data kasus dengan lengkap untuk verifikasi
                                DPJP
                            </p>
                        </div>
                    </div>
                ...
                <div class="space-y-4 text-xs sm:text-sm">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            >
                                ...
                                >
                                    {{ d.name }} ({{
                                        d.specialization?.name || 'Spesialis'
                                    }})
                                </option>
                            </select>
                            <p
                                v-if="formErrors.doctor_id"
                                class="mt-1 text-xs text-rose-600"
                            >
                    ...
                    <div>
                        <label class="mb-1 block font-bold text-[#000000]"
                            >Temuan Klinis & Hasil Pemeriksaan *</label
                        >
                        <textarea
                        ...
                        ></textarea>
                        <p
                            v-if="formErrors.clinical_findings"
                            class="mt-1 text-xs text-rose-600"
                        >
                            {{ formErrors.clinical_findings }}
                        </p>
                    </div>
                    ...
                    <!-- Refleksi Pembelajaran -->
                    <div>
                        <label class="mb-1 block font-bold text-[#065f46]"
                            >Refleksi Pembelajaran Mandiri (Learning Reflection)
                            *</label
                        >
                        <textarea
                        ...
                        ></textarea>
                        <p
                            v-if="formErrors.learning_reflection"
                            class="mt-1 text-xs text-rose-600"
                        >
                            {{ formErrors.learning_reflection }}
                        </p>
                    </div>
                </div>
                ...
                <!-- Footer Buttons -->
                <div
                    class="flex flex-col-reverse items-center justify-end gap-3 border-t border-[#000000]/10 pt-4 sm:flex-row"
                >
                    <button
                        type="button"
                        @click="closeModal"
                        class="min-h-[44px] w-full rounded-xl border border-[#000000]/15 px-5 py-2.5 text-xs font-semibold text-[#000000]/70 transition-colors hover:bg-[#edede2] sm:w-auto"
                    >
                        Batal
                    </button>
                    ...
                    <button
                        ...
                        class="min-h-[44px] w-full rounded-xl border border-[#065f46] bg-[#fffff3] px-5 py-2.5 text-xs font-bold text-[#065f46] transition-colors hover:bg-[#beedc0]/30 sm:w-auto"
                    >
                        Simpan Draft
                    </button>
                    ...
                    <button
                        ...
                        :disabled="isSubmitting"
                        class="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-[#065f46] px-6 py-2.5 text-xs font-bold text-[#ffffff] shadow-sm transition-colors hover:bg-[#054d38] sm:w-auto"
                    >
                        <Loader2
                            v-if="isSubmitting"
        ...
        <div
            v-if="selectedFeedbackItem"
            class="fixed inset-0 z-50 flex items-center justify-center bg-[#000000]/50 p-4 backdrop-blur-xs"
        >
            <motion.div
                :initial="{ opacity: 0, scale: 0.95 }"
                :animate="{ opacity: 1, scale: 1 }"
                class="w-full max-w-lg space-y-4 rounded-3xl border border-purple-200 bg-[#fffff3] p-6 shadow-2xl sm:p-8"
            >
                <div
                ...
                >
                    <div class="flex items-center gap-2.5">
                        <div
                            class="flex size-9 items-center justify-center rounded-xl bg-purple-100 text-purple-800"
                        >
                            <Award class="size-5" />
                        </div>
                        <div>
                            ...
                            >
                                Evaluasi & Feedback DPJP
                            </h3>
                            <p class="text-xs text-[#000000]/60">
                                {{
                                    selectedFeedbackItem.doctor?.name ||
                                    'Dokter Spesialis'
                                }}
                            </p>
                        </div>
                ...
                <div class="space-y-3 text-xs">
                    <div
                        v-if="selectedFeedbackItem.score !== null"
                        class="flex items-center justify-between rounded-xl bg-[#065f46] p-4 text-[#ffffff]"
                    >
                        <div>
                            <div
                                class="text-xs font-bold text-[#beedc0] uppercase"
                            >
                                Skor Nilai Kasus
                            </div>
                            <div class="font-mono text-2xl font-bold">
                                {{ selectedFeedbackItem.score }} / 100
                            </div>
                        </div>
                    ...
                    <div
                        class="space-y-1.5 rounded-xl border border-purple-200 bg-purple-50/60 p-4"
                    >
                        <div class="font-bold text-purple-900">
                            Catatan & Umpan Balik:
                        </div>
                        <p class="leading-relaxed text-purple-950">
                            {{ selectedFeedbackItem.supervisor_feedback }}
                        </p>
                    </div>
                    ...
                    <div class="text-right text-xs text-[#000000]/50">
                        Ditinjau pada:
                        {{
                            selectedFeedbackItem.reviewed_at
                                ? new Date(
                                      selectedFeedbackItem.reviewed_at,
