...
const openReviewModal = (item: ClinicalLogbookSupervisionItem) => {
    ...
    reviewErrors.value = {}
    reviewForm.value = {
        status: item.status === 'revision_needed' ? 'revision_needed' : 'approved',
...
const submitReview = async () => {
    ...
    try {
        await axios.post(`/doctor/supervision/${selectedLogbook.value.clinical_logbook_id}/review`, reviewForm.value)
        closeReviewModal()
        router.reload()
    } catch (err: any) {
...
const statusBadges: Record<string, { label: string; badge: string; dot: string }> = {
    draft: { label: 'Draft', badge: 'bg-neutral-100 text-neutral-600 border-neutral-200', dot: 'bg-neutral-400' },
    submitted: { label: 'Menunggu Verifikasi DPJP', badge: 'bg-amber-50 text-amber-800 border-amber-300 animate-pulse font-bold', dot: 'bg-amber-500' },
    approved: { label: 'Telah Disetujui (Sign-off)', badge: 'bg-emerald-50 text-emerald-800 border-emerald-300 font-bold', dot: 'bg-emerald-600' },
    revision_needed: { label: 'Perlu Revisi Mahasiswa', badge: 'bg-rose-50 text-rose-800 border-rose-300 font-bold', dot: 'bg-rose-600' },
}
</script>
        ...
        <div class="mx-auto max-w-7xl space-y-6">
            ...
            >
                <div class="space-y-1">
                    ...
                    <p class="text-xs text-[#000000]/60 sm:text-sm">
                        Verifikasi keakuratan temuan klinis, berikan umpan balik edukatif, nilai kompetensi mahasiswa, dan tandatangani logbook.
                    </p>
                </div>
            </motion.div>
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div class="rounded-xl border border-[#000000]/10 bg-[#fffff3] p-4 shadow-sm">
                    <div class="text-xs font-semibold text-[#000000]/60 uppercase">Total Logbook Ditugaskan</div>
                    <div class="mt-1 text-2xl font-bold font-mono text-[#000000]">{{ stats.total_assigned }}</div>
                </div>
                <div class="rounded-xl border border-amber-200 bg-amber-50/60 p-4 shadow-sm">
                    <div class="text-xs font-semibold text-amber-800 uppercase">Menunggu Review</div>
                    <div class="mt-1 text-2xl font-bold font-mono text-amber-800">{{ stats.pending_review }}</div>
                </div>
                <div class="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 shadow-sm">
                    <div class="text-xs font-semibold text-emerald-800 uppercase">Telah Disetujui</div>
                ...
                <div class="rounded-xl border border-rose-200 bg-rose-50/60 p-4 shadow-sm">
                    <div class="text-xs font-semibold text-rose-800 uppercase">Diminta Revisi</div>
                    <div class="mt-1 text-2xl font-bold font-mono text-rose-800">{{ stats.revision_needed }}</div>
                </div>
            </div>
            ...
            <div class="space-y-4" v-if="logbooks.data.length > 0">
                <motion.div
                    ...
                    :animate="{ opacity: 1, y: 0 }"
                    class="rounded-2xl border bg-[#fffff3] p-6 shadow-sm transition-all flex flex-col justify-between gap-4"
                    :class="item.status === 'submitted' ? 'border-amber-300 ring-2 ring-amber-100' : 'border-[#000000]/10 hover:border-[#065f46]/30'"
                >
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="space-y-1.5 flex-1">
                            ...
                            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[#000000]/60">
                                <span>Mahasiswa Koas: <strong class="text-[#065f46]">{{ item.nurse?.name || 'Mahasiswa' }}</strong> (NIM: {{ item.nurse?.registration_number || '-' }})</span>
                                <span>•</span>
                                <span>Pasien: <strong class="text-[#000000]">{{ item.patient?.name || 'Pasien' }}</strong></span>
                                <span>•</span>
                                <span>Diajukan: {{ item.submitted_at ? new Date(item.submitted_at).toLocaleDateString('id-ID', { dateStyle: 'medium' }) : '-' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 self-end sm:self-start">
                            <motion.button
                                ...
                                class="min-h-[44px] inline-flex items-center gap-2 rounded-xl bg-[#065f46] px-5 py-2.5 text-xs font-bold text-[#ffffff] shadow-sm hover:bg-[#054d38] transition-colors cursor-pointer"
                            >
                                <FileCheck class="size-4 text-[#beedc0]" />
                                <span>{{ item.status === 'approved' ? 'Ubah Evaluasi' : 'Supervisi & Sign-off' }}</span>
                            </motion.button>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl bg-[#edede2]/50 p-4 text-xs text-[#000000]/80 space-y-2 border border-[#000000]/5">
                        <div>
                            <span class="font-bold text-[#000000]">Temuan Klinis Mahasiswa: </span>
                            <span>{{ item.clinical_findings }}</span>
                        </div>
                        <div v-if="item.procedure_performed">
                            <span class="font-bold text-[#000000]">Tindakan Medis Dilakukan: </span>
                        ...
                        <div v-if="item.supervisor_feedback" class="pt-2 border-t border-[#000000]/10 text-purple-900">
                            <span class="font-bold">Feedback DPJP Sebelumnya: </span>
            ...
            <div class="rounded-2xl border border-dashed border-[#000000]/20 bg-[#fffff3] p-12 text-center" v-else>
                <GraduationCap class="size-12 text-[#000000]/30 mx-auto mb-3" />
                <h3 class="font-serif text-lg font-bold text-[#000000]">Tidak Ada Logbook Menunggu Supervisi</h3>
                <p class="text-xs text-[#000000]/60 max-w-md mx-auto mt-1">
                    Semua kasus klinis yang diajukan oleh mahasiswa koas di bawah bimbingan Anda telah selesai ditinjau.
                </p>
            </div>
        </div>

        <!-- Modal Supervisi & Dual Sign-off DPJP -->
        <div v-if="isReviewModalOpen && selectedLogbook" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#000000]/50 backdrop-blur-xs">
            <motion.div
            ...
            >
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-[#000000]/60">
                                Mahasiswa: <strong class="text-[#065f46]">{{ selectedLogbook.nurse?.name }}</strong> ({{ selectedLogbook.nurse?.registration_number }})
                            </p>
                        </div>
                    </div>
                    <button type="button" @click="closeReviewModal" class="p-2 text-[#000000]/50 hover:text-[#000000]">
                        <X class="size-5" />
                    </button>
                </div>

                <!-- Ringkasan Kasus Koas -->
                <div class="rounded-xl bg-[#edede2]/60 p-4 space-y-2 text-xs border border-[#000000]/10">
                    <div class="font-bold font-serif text-sm text-[#000000]">
                        {{ selectedLogbook.case_title }}
                    </div>
                    <div class="text-[#000000]/70">
                        Pasien: {{ selectedLogbook.patient?.name }} (NIK: {{ selectedLogbook.patient?.resident_n }})
                    </div>
                    <div class="text-[#000000]/90">
                        <strong>Temuan: </strong> {{ selectedLogbook.clinical_findings }}
                    </div>
                    <div class="text-[#000000]/90 italic">
                        <strong>Refleksi: </strong> "{{ selectedLogbook.learning_reflection }}"
                    </div>
                </div>
                ...
                <div class="space-y-4 text-xs sm:text-sm">
                    ...
                    <div>
                        <label class="block font-bold text-[#000000] mb-2">Keputusan Supervisi DPJP *</label>
                        <div class="grid grid-cols-2 gap-3">
                            <label
                                :class="reviewForm.status === 'approved' ? 'border-[#065f46] bg-[#beedc0]/20 ring-1 ring-[#065f46]' : 'border-[#000000]/15 bg-[#fffff3]'"
                            ...
                            >
                                ...
                                />
                                <div>
                                    <div class="font-bold text-[#065f46]">Setujui (Approved)</div>
                                    <div class="text-xs text-[#000000]/60">Dual sign-off selesai & valid</div>
                                </div>
                            </label>

                            <label
                                :class="reviewForm.status === 'revision_needed' ? 'border-rose-500 bg-rose-50 ring-1 ring-rose-500' : 'border-[#000000]/15 bg-[#fffff3]'"
                                class="flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all"
                            >
                                <input
                                    type="radio"
                                ...
                                />
                                <div>
                                    <div class="font-bold text-rose-800">Minta Revisi</div>
                                    <div class="text-xs text-[#000000]/60">Mahasiswa wajib memperbaiki</div>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block font-bold text-[#000000] mb-1">Skor Nilai Kasus (0 - 100)</label>
                        <input
                        ...
                        />
                        <p v-if="reviewErrors.score" class="text-xs text-rose-600 mt-1">{{ reviewErrors.score }}</p>
                    </div>

                    <!-- Feedback & Catatan Edukatif DPJP -->
                    <div>
                        <label class="block font-bold text-[#000000] mb-1">Catatan Evaluasi & Umpan Balik Klinis *</label>
                        <textarea
                        ...
                        ></textarea>
                        <p v-if="reviewErrors.supervisor_feedback" class="text-xs text-rose-600 mt-1">{{ reviewErrors.supervisor_feedback }}</p>
                    </div>
                </div>

                <!-- Footer Buttons -->
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-[#000000]/10">
                    <button
                        type="button"
                        @click="closeReviewModal"
                        class="min-h-[44px] px-5 py-2.5 rounded-xl border border-[#000000]/15 text-xs font-semibold text-[#000000]/70 hover:bg-[#edede2] transition-colors"
                    >
                        Batal
                    </button>
                    ...
                    <button
                        ...
                        class="min-h-[44px] px-6 py-2.5 rounded-xl bg-[#065f46] text-xs font-bold text-[#ffffff] hover:bg-[#054d38] transition-colors flex items-center gap-2 shadow-sm"
                    >
