...
const handleVideoEnded = () => {
    ...
    if (activeVideos.value.length === 1) {
        ...
        if (currentVideo.value?.youtube_id && ytPlayer && typeof ytPlayer.seekTo === 'function') {
            try {
                ytPlayer.seekTo(0);
                ytPlayer.playVideo();
            } catch {}
    ...
    currentVideoIndex.value = (currentVideoIndex.value + 1) % activeVideos.value.length;
    playCurrentVideo();
};
...
<template>
    <Head :title="`Layar Monitor Antrean & Video Display TV - ${displayConfig?.hospital_name || 'Hospital Population'}`" />
    ...
    >
        ...
        >
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    >
                        {{ displayConfig?.hospital_name || 'Hospital Population' }}
                    </h1>
                    <div class="flex items-center gap-2 text-xs text-[#333333] font-['Rubik']">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <button
                    ...
                    :title="isAudioEnabled ? 'Panggilan suara antrean aktif' : 'Panggilan suara dinonaktifkan'"
                >
                    <component
                    ...
                    />
                    <span>{{
                        isAudioEnabled ? 'Suara Panggilan Aktif' : 'Suara Senyap'
                    }}</span>
                </button>
                ...
                <div class="border-l border-[#000000]/15 pl-3 text-right">
                    ...
                    <span class="mt-1 block text-[11px] font-medium text-[#333333]">
                        {{ currentDate }}
                    </span>
                </div>
            </div>
        ...
        <main class="my-4 grid flex-1 grid-cols-1 gap-5 lg:grid-cols-12">
            ...
            <div class="flex flex-col gap-5 lg:col-span-7">
                ...
                >
                    ...
                    <div class="relative aspect-video w-full overflow-hidden bg-neutral-950">
                        ...
                        >
                            <div id="yt-player-target" class="h-full w-full"></div>
                        </div>
                        ...
                        >
                            <div class="flex size-16 items-center justify-center rounded-full bg-[#beedc0]/20 text-[#beedc0]">
                                <Tv class="size-8" />
                            </div>
                            <h3 class="font-['ivypresto-headline'] text-2xl font-bold text-[#ffffff]">
                                Layar Edukasi & Informasi Rumah Sakit
                            </h3>
                            <p class="max-w-md text-xs text-[#ffffff]/70 font-['Rubik']">
                                Menampilkan informasi kesehatan terpercaya dan panduan layanan medis poliklinik untuk kenyamanan pasien dan keluarga.
                            </p>
                        </div>
                        ...
                        >
                            <div class="flex items-center gap-2.5 truncate">
                                <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-[#beedc0] text-[#000000]">
                                    <Film class="size-3" />
                                </span>
                                <div class="truncate">
                                    <h4 class="truncate text-xs font-bold text-[#ffffff] sm:text-sm font-['Rubik']">
                                        {{ currentVideo.title }}
                                    </h4>
                                    <span class="text-[10px] text-[#beedc0]">
                                        Video {{ currentVideoIndex + 1 }} dari {{ activeVideos.length }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                :title="isVideoMuted ? 'Aktifkan suara video' : 'Senyapkan suara video'"
                            >
                                <component
                                    :is="isVideoMuted ? VolumeX : Volume2"
                                    class="size-3.5"
                                    :class="isVideoMuted ? 'text-[#ffffff]/70' : 'text-[#beedc0]'"
                                />
                                <span class="hidden sm:inline">{{ isVideoMuted ? 'Audio Bisu' : 'Audio Aktif' }}</span>
                            </button>
                        </div>
                    </div>
                </motion.div>
                ...
                >
                    <div class="flex items-center justify-between border-b border-[#ffffff]/15 pb-3.5">
                        <div class="flex items-center gap-2">
                            <span class="size-3 rounded-full bg-[#beedc0] animate-ping"></span>
                            <span class="text-xs font-bold tracking-widest text-[#beedc0] uppercase">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span v-else class="text-xs text-[#ffffff]/60 font-mono">
                                Real-Time Call
                            </span>
                        </div>
                    </div>
                    ...
                    >
                        <span class="block text-xs font-semibold tracking-wider text-[#ffffff]/60 uppercase">
                            Nomor Antrean Pasien
                        </span>
                        <span
                            class="block font-mono text-6xl font-black tracking-tighter text-[#beedc0] sm:text-7xl lg:text-8xl animate-pulse"
                        >
                            {{ displayData.latestCalled.queue_number }}
                        </span>
                        <div class="space-y-1 pt-2">
                            <h2 class="font-['ivypresto-headline'] text-2xl font-bold text-[#ffffff] sm:text-3xl">
                                {{ displayData.latestCalled.poli_name }} &bull; {{ displayData.latestCalled.room_name }}
                            </h2>
                            <p class="text-base font-semibold text-[#ffffff]/90 sm:text-lg">
                                {{ displayData.latestCalled.patient_name }}
                            </p>
                            <span class="block text-xs font-medium text-[#beedc0]">
                    ...
                    <div v-else class="my-auto space-y-3 py-10 text-center">
                        <div class="mx-auto flex size-16 items-center justify-center rounded-2xl bg-[#ffffff]/10 text-[#beedc0]">
                            <Bell class="size-8 animate-bounce" />
                        </div>
                        <div class="space-y-1">
                            <h3 class="font-['ivypresto-headline'] text-2xl font-bold text-[#ffffff] sm:text-3xl">
                                Menunggu Panggilan Dokter
                            </h3>
                            <p class="mx-auto max-w-sm text-xs font-['Rubik'] text-[#ffffff]/70">
                                Nomor antrean dan ruang periksa akan otomatis terpampang dan dipanggil melalui suara saat dokter memanggil pasien.
                            </p>
                        </div>
                        <div class="inline-flex items-center gap-2 rounded-full border border-[#beedc0]/30 bg-[#beedc0]/10 px-4 py-1.5 text-xs font-semibold text-[#beedc0]">
                            <Activity class="size-3.5 animate-pulse" />
                            <span>Sistem Pemanggilan Antrean Terpadu</span>
                        </div>
                    </div>

                    <div class="rounded-2xl border border-[#ffffff]/10 bg-[#ffffff]/5 p-3 text-center text-xs text-[#ffffff]/80 font-['Rubik']">
                        Silakan mempersiapkan berkas pendaftaran dan langsung menuju ke ruangan saat nomor Anda dipanggil.
                    </div>
                </motion.div>
            </div>
            ...
            <motion.div
                ...
                class="flex flex-col rounded-3xl border border-[#000000]/10 bg-[#fffff3] p-5 shadow-none sm:p-6 lg:col-span-5 h-full max-h-[85vh] min-h-[600px] overflow-hidden"
            >
                <!-- Header Matriks Poliklinik -->
                <div class="mb-4 flex items-center justify-between border-b border-[#000000]/10 pb-3.5 shrink-0">
                    <div class="flex items-center gap-2.5">
                        <div class="flex size-9 items-center justify-center rounded-xl bg-[#000000] text-[#beedc0] shadow-xs">
                            <Radio class="size-4.5" />
                        </div>
                        <div>
                            <h3 class="font-['ivypresto-headline'] text-lg font-bold text-[#000000] sm:text-xl leading-tight">
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-full border border-[#000000]/15 bg-[#edede2] px-3 py-1 text-xs font-bold text-[#000000]">
                        <span class="size-2 rounded-full bg-emerald-500 animate-pulse"></span>
                ...
                >
                    <motion.div
                        ...
                        :animate="{ opacity: 1, y: 0 }"
                        class="flex flex-col justify-between rounded-2xl border transition-all shadow-xs bg-[#ffffff]"
                        :class="[
                            clinic.current_calling
                                ? 'border-l-4 border-l-emerald-500 border-t-[#000000]/10 border-r-[#000000]/10 border-b-[#000000]/10 ring-1 ring-emerald-500/20'
                                : clinic.waiting_count > 0
                                  ? 'border-l-4 border-l-amber-400 border-t-[#000000]/10 border-r-[#000000]/10 border-b-[#000000]/10'
                                  : 'border-l-4 border-l-neutral-300 border-t-[#000000]/10 border-r-[#000000]/10 border-b-[#000000]/10'
                        ]"
                    >
                        <div class="p-3.5 sm:p-4 space-y-2.5">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="min-w-0 flex-1 space-y-1">
                                    <div class="flex items-center gap-2">
                                        <h4 class="font-['ivypresto-headline'] text-base font-bold text-[#000000] truncate sm:text-lg">
                                            {{ clinic.poli_name }}
                                        </h4>
                                    </div>
                                    <div class="flex items-center gap-1.5 text-xs text-[#333333] font-medium truncate">
                                        <Stethoscope class="size-3.5 shrink-0 text-[#000000]" />
                                        <span class="truncate">{{ clinic.doctor_name }}</span>
                                    </div>
                                    <div class="pt-0.5">
                                        <span class="inline-flex items-center rounded-md bg-[#edede2] px-2 py-0.5 text-[10px] sm:text-[11px] font-semibold text-[#000000]">
                                            {{ clinic.room_name }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="shrink-0 text-right min-w-[110px]">
                                    ...
                                    <div v-if="clinic.current_calling" class="flex flex-col items-end">
                                        <span class="inline-flex items-center gap-1 rounded-full bg-[#beedc0] px-2.5 py-0.5 text-[10px] font-extrabold text-[#000000] tracking-wide uppercase">
                                            <span class="size-1.5 rounded-full bg-emerald-700 animate-ping"></span>
                                            <span>Diperiksa</span>
                                        </span>
                                        <div class="mt-1 font-mono text-2xl font-black text-[#000000] sm:text-3xl tracking-tight">
                                            {{ clinic.current_calling }}
                                        </div>
                                    </div>

                                    <!-- Status 2: Menunggu Panggilan Dokter -->
                                    <div v-else-if="clinic.waiting_count > 0" class="flex flex-col items-end">
                                        <span class="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-0.5 text-[10px] font-bold text-amber-900 tracking-wide uppercase">
                                            <Clock class="size-3 text-amber-700" />
                                            <span>Menunggu</span>
                                        </span>
                                        <div class="mt-1 font-mono text-xl font-bold text-[#333333] sm:text-2xl">
                                            {{ clinic.next_calling !== '-' ? clinic.next_calling : `${clinic.waiting_count} Siap` }}
                                        </div>
                                    </div>

                                    <!-- Status 3: Standby / Antrean Kosong -->
                                    <div v-else class="flex flex-col items-end">
                                        <span class="inline-flex items-center gap-1 rounded-full border border-[#000000]/10 bg-[#edede2] px-2.5 py-0.5 text-[10px] font-bold text-[#333333] tracking-wide uppercase">
                                            <CheckCircle2 class="size-3 text-emerald-600" />
                                            <span>Standby</span>
                                        </span>
                                        <div class="mt-1 font-mono text-xs sm:text-sm font-semibold text-[#333333]/60">
                                            Siap Melayani
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-between border-t border-[#000000]/5 pt-2 text-xs text-[#333333] font-['Rubik']">
                                <div class="flex items-center gap-1.5 text-[11px] sm:text-xs">
                                    <span class="text-[#333333]/70">Berikutnya:</span>
                                    <span
                                        class="font-mono font-bold"
                                        :class="clinic.next_calling && clinic.next_calling !== '-' ? 'text-[#000000]' : 'text-[#333333]/40'"
                                    >
                                        {{ clinic.next_calling && clinic.next_calling !== '-' ? clinic.next_calling : '--' }}
                                    </span>
                                </div>
                                ...
                                <span
                                    :class="[
                                        clinic.waiting_count > 0
                                            ? 'bg-[#beedc0] text-[#000000] font-bold'
                                ...
                                >
                                    <Users class="size-3" />
                                    <span>{{ clinic.waiting_count }} Menunggu</span>
                                </span>
                            </div>
                        </div>
                    </motion.div>
                ...
                <div
                    v-else
                    class="flex flex-1 flex-col items-center justify-center p-8 text-center space-y-3"
                >
                    <div class="flex size-14 items-center justify-center rounded-2xl bg-[#edede2] text-[#000000]">
                        <Stethoscope class="size-7 text-[#000000]" />
                    </div>
                    <div>
                        <h4 class="font-['ivypresto-headline'] text-lg font-bold text-[#000000]">
                            Tidak Ada Poliklinik Aktif
                        </h4>
                        <p class="text-xs text-[#333333] max-w-xs font-['Rubik'] mt-1">
                            Jadwal poliklinik dan dokter yang bertugas hari ini akan otomatis muncul pada layar monitor.
                        </p>
                    </div>
                </div>
            </motion.div>
        ...
        >
            <div class="flex shrink-0 items-center gap-2 border-r border-[#ffffff]/20 pr-4 font-bold text-[#beedc0] text-xs uppercase">
                <Sparkles class="size-4" />
                <span>Pengumuman</span>
            </div>
            <div class="relative flex-1 overflow-hidden pl-4 text-xs sm:text-sm font-['Rubik'] text-[#ffffff]/90">
