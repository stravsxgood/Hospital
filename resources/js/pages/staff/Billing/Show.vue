...
// QRIS State
const qrString = ref<string | null>(
    props.billing.payment_method === 'xendit_qris'
        ? (props.billing.xendit_payment_url ?? null)
        : null,
);
...
const xenditInvoiceUrl = ref<string | null>(
    props.billing.xendit_payment_url || null,
);
...
const cashChange = computed(() =>
    Math.max(0, cashReceived.value - totalBill.value),
);
...
<template>
    <AppSidebarLayout>
        ...
        >
            ...
            <div
                v-if="errorMessage"
                class="mb-6 flex items-center gap-2 rounded-xl border border-rose-300 bg-rose-50 p-4 text-sm text-rose-800"
            >
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div
                        class="rounded-2xl border border-[#333333]/15 bg-[#fffff3] p-5 shadow-sm"
                    >
                        <h3
                            class="mb-4 text-sm font-bold tracking-wider text-neutral-500 uppercase"
                        >
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="space-y-1">
                                ...
                                <div class="text-xs text-neutral-600">
                                    NIK: {{ billing.patient?.resident_n }}
                                </div>
                            </div>
                            <div class="space-y-1">
                                <span class="text-xs text-neutral-500"
                                ...
                                <div class="font-medium text-[#000000]">
                                    {{ billing.patient?.number_phone || '-' }}
                                </div>
                                <div class="text-xs text-neutral-600">
                                    {{ billing.patient?.gender }} ·
                                    {{ billing.patient?.address || 'Jakarta' }}
                                </div>
                            </div>
                            <div class="space-y-1">
                                <span class="text-xs text-neutral-500"
                                    >Poliklinik Tujuan:</span
                                >
                                <div class="font-semibold text-[#000000]">
                                    {{
                                        billing.reservation?.doctor_schedule
                                            ?.poli?.name_poli ?? 'Poliklinik'
                                    }}
                                </div>
                                <div class="text-xs text-neutral-600">
                                    Ruang:
                    ...
                    >
                        <div
                            class="border-b border-[#333333]/15 bg-[#edede2]/60 px-5 py-3.5 font-bold text-[#000000]"
                        >
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm">
                                <thead
                                    class="border-b border-neutral-200 bg-neutral-50 text-xs font-semibold tracking-wider text-neutral-600 uppercase"
                                >
                                    <tr>
                                ...
                                <tbody class="divide-y divide-neutral-200">
                                    <tr
                                        ...
                                        :key="item.billing_item_id"
                                    >
                                        <td
                                            class="px-5 py-3.5 text-center text-xs text-neutral-500"
                                        >
                                            {{ idx + 1 }}
                                        </td>
                                        <td class="px-5 py-3.5">
                                            ...
                                            <span
                                                class="text-[11px] font-semibold text-neutral-500 uppercase"
                                            >
                                                {{
                                        ...
                                        <td
                                            class="px-5 py-3.5 text-center font-semibold text-neutral-700"
                                        >
                                            {{ item.quantity }}
                                        </td>
                                        <td
                                            class="px-5 py-3.5 text-right font-medium text-neutral-700"
                                        >
                                            {{ formatRupiah(item.unit_price) }}
                                        </td>
                                        <td
                                            class="px-5 py-3.5 text-right font-bold text-[#000000]"
                                        >
                                ...
                                <tfoot
                                    class="border-t-2 border-neutral-300 bg-[#edede2]/30"
                                >
                                    <tr>
                                        <td
                                            colspan="4"
                                            class="px-5 py-4 text-right text-base font-bold text-[#000000]"
                                        >
                                            TOTAL TAGIHAN:
                                        </td>
                                        <td
                                            class="px-5 py-4 text-right font-['ivypresto-headline'] text-xl font-extrabold text-[#000000]"
                                        >
                                            {{
                                                formatRupiah(
                ...
                <div class="space-y-6">
                    <!-- Status Card if Paid -->
                    <div
                        v-if="billing.status === 'paid' || isPaidSuccess"
                        class="rounded-2xl border border-emerald-300 bg-emerald-50/80 p-6 text-center shadow-sm"
                    >
                        <div
                            class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"
                        >
                        ...
                        <div
                            class="mt-4 space-y-1.5 rounded-xl border border-emerald-200 bg-white p-3.5 text-left text-xs text-neutral-700"
                        >
                            ...
                            <div class="flex justify-between">
                                ...
                                >
                                <span class="font-semibold">{{
                                    billing.processed_by_nurse?.name ??
                                    'Staf Kasir'
                                }}</span>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <span
                                class="rounded-full bg-[#beedc0]/50 px-2 py-0.5 text-[10px] font-bold text-[#065f46]"
                            >
                                Multi-Channel
                            </span>
                        </div>
                        ...
                        >
                            <div
                                v-if="!qrString"
                                class="rounded-xl border border-neutral-200 bg-white p-4 text-center"
                            >
                                <p class="text-xs text-neutral-600">
                                    Tampilkan Barcode QRIS Dinamis langsung di
                                    layar untuk dipindai oleh pasien:
                                </p>
                                <button
                                    type="button"
                            ...
                            <div
                                v-else
                                class="rounded-xl border border-neutral-200 bg-white p-3 text-center"
                            >
                                <div
                                    class="mb-2 flex items-center justify-between border-b border-neutral-200 pb-1.5 text-[11px]"
                                >
                                    <span class="font-bold text-rose-700"
                                        >QRIS GPN</span
                                    >
                                    <span
                                        class="flex items-center gap-1 font-semibold text-emerald-700"
                                    >
                                        <span
                                ...
                                <div
                                    class="mt-2 flex items-center justify-center gap-1.5 rounded-lg bg-amber-50 p-2 text-[11px] text-amber-800"
                                >
                                    <Loader2 class="h-3.5 w-3.5 animate-spin" />
                                    <span
                                        >Menunggu pembayaran pasien via
                                        m-banking...</span
                                    >
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <div
                                class="space-y-1 rounded-xl border border-neutral-200 bg-white p-3 text-xs"
                            >
                                <div
                                ...
                                <div
                                    class="flex justify-between border-t border-neutral-200 pt-1.5 text-xs font-bold"
                                >
                                    <span
                                        :class="
                            ...
                            <button
                                type="button"
                                :disabled="
                                    cashReceived < totalBill || isSubmitting
                                "
                        ...
                        <!-- 3. EDC VIEW -->
                        <div
                            v-else-if="selectedMethod === 'edc'"
                            class="mt-4 space-y-3"
                        >
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label
                                        class="block text-[11px] font-semibold text-neutral-700"
                                        >Tipe</label
                                    >
                                    <select
                        ...
                        <!-- 4. VA VIEW -->
                        <div
                            v-else-if="selectedMethod === 'va'"
                            class="mt-4 space-y-3"
                        >
                            <div
                                v-if="xenditInvoiceUrl"
                                class="rounded-xl border border-emerald-300 bg-emerald-50 p-3 text-center"
                            >
                                <span class="text-xs font-bold text-emerald-900"
                                    >Tautan VA Aktif</span
                                >
                                <div class="mt-2">
                                    <a
