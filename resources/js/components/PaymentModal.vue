...
watch(
    ...
    (open) => {
        if (open && props.billing) {
            ...
            if (props.billing.payment_method === 'xendit_qris' && props.billing.xendit_payment_url) {
...
<template>
    ...
    >
        <motion.div
            ...
            class="relative flex w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-[#333333]/20 bg-[#fffff3] shadow-2xl font-['Rubik'] text-[#000000]"
        >
            <!-- ═══════════════════════════════════════════════════════════════
                 Modal Header
                 ═══════════════════════════════════════════════════════════════ -->
            <div class="flex items-center justify-between border-b border-[#333333]/15 bg-[#edede2]/70 px-6 py-4">
                <div class="flex items-center gap-2.5">
                    <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-[#000000] text-white">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-neutral-600">
                            Invoice: <strong class="text-neutral-900">{{ billing.invoice_number }}</strong> · {{ billing.patient?.name }}
                        </p>
                    </div>
                </div>
            ...
            <!-- Error Banner -->
            <div v-if="errorMessage" class="mx-6 mt-4 flex items-center gap-2 rounded-xl border border-rose-300 bg-rose-50 p-3 text-xs text-rose-800">
            ...
            <div v-if="isPaidSuccess" class="p-8 text-center">
                ...
                <h3 class="font-['ivypresto-headline'] text-2xl font-bold text-[#000000]">
                    Pembayaran Berhasil & Lunas!
                </h3>
                <p class="mt-1 text-xs text-neutral-600">
                    Transaksi telah diverifikasi ke dalam sistem SIMRS & antrean pasien otomatis diperbarui.
                </p>

                <div class="mx-auto mt-6 max-w-md rounded-2xl border border-[#333333]/15 bg-[#edede2]/40 p-4 text-left text-xs space-y-2">
                    <div class="flex justify-between">
                        <span class="text-neutral-500">Nomor Invoice:</span>
                        <span class="font-bold text-neutral-900">{{ billing.invoice_number }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-neutral-500">Nama Pasien:</span>
                        <span class="font-semibold text-neutral-900">{{ billing.patient?.name }}</span>
                    </div>
                    <div class="flex justify-between border-t border-neutral-300 pt-2 font-bold text-sm">
                        <span>Total Pelunasan:</span>
                        <span class="text-[#065f46]">{{ formatRupiah(totalBill) }}</span>
                    </div>
                </div>

                <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
                    <a
                        :href="paidReceiptUrl || `/staff/billing/${billing.billing_id}/print-receipt?stream=1`"
                        target="_blank"
                        class="inline-flex min-h-[44px] w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-[#000000] px-6 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-[#333333]"
                    >
                    ...
                    <button
                        type="button"
                        class="inline-flex min-h-[44px] w-full sm:w-auto items-center justify-center rounded-xl border border-neutral-300 bg-white px-5 py-2.5 text-sm font-medium text-neutral-700 hover:bg-neutral-100"
                        @click="emit('close')"
                    >
                        Tutup Panel Kasir
                    </button>
            ...
            <div v-else class="max-h-[80vh] overflow-y-auto p-6">
                <!-- Total Amount Banner -->
                <div class="mb-5 flex items-center justify-between rounded-2xl border border-[#beedc0] bg-[#beedc0]/25 p-4">
                    <div>
                        <span class="text-xs font-semibold text-[#065f46] uppercase tracking-wider">Total Tagihan Rawat Jalan</span>
                        <div class="font-['ivypresto-headline'] text-2xl font-black text-[#000000] sm:text-3xl">
                            {{ formatRupiah(totalBill) }}
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="rounded-full border border-[#beedc0] bg-white px-3 py-1 text-[11px] font-bold text-[#065f46]">
                ...
                <!-- ═══════════════════════════════════════════════════════════
                     ...
                     ═══════════════════════════════════════════════════════════ -->
                <div class="mb-5">
                    <label class="mb-2 block text-xs font-bold uppercase tracking-wider text-neutral-500">
                    ...
                    <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                        ...
                        <button
                            type="button"
                            :class="selectedMethod === 'qris' ? 'border-[#000000] bg-[#000000] text-white shadow-md' : 'border-neutral-300 bg-white text-neutral-800 hover:border-neutral-400 hover:bg-neutral-50'"
                        ...
                        >
                            <div class="flex items-center justify-between">
                                <QrCode :class="selectedMethod === 'qris' ? 'text-[#beedc0]' : 'text-neutral-700'" class="h-5 w-5" />
                                <span :class="selectedMethod === 'qris' ? 'bg-[#beedc0] text-[#000000]' : 'bg-emerald-100 text-emerald-800'" class="rounded px-1.5 py-0.5 text-[9px] font-bold uppercase">
                                    Instant
                                </span>
                            </div>
                            <div>
                                <div class="font-bold text-xs">QRIS Dinamis</div>
                                <div :class="selectedMethod === 'qris' ? 'text-neutral-300' : 'text-neutral-500'" class="text-[10px]">
                        ...
                        <button
                            type="button"
                            :class="selectedMethod === 'cash' ? 'border-[#000000] bg-[#000000] text-white shadow-md' : 'border-neutral-300 bg-white text-neutral-800 hover:border-neutral-400 hover:bg-neutral-50'"
                            class="flex min-h-[72px] flex-col justify-between rounded-xl border p-3 text-left transition"
                            @click="selectedMethod = 'cash'"
                        >
                            <div class="flex items-center justify-between">
                                <DollarSign :class="selectedMethod === 'cash' ? 'text-[#beedc0]' : 'text-neutral-700'" class="h-5 w-5" />
                                <span :class="selectedMethod === 'cash' ? 'bg-white/20 text-white' : 'bg-neutral-100 text-neutral-700'" class="rounded px-1.5 py-0.5 text-[9px] font-semibold">
                                    Fisik
                                </span>
                            </div>
                            <div>
                                <div class="font-bold text-xs">Tunai (Cash)</div>
                                <div :class="selectedMethod === 'cash' ? 'text-neutral-300' : 'text-neutral-500'" class="text-[10px]">
                                    Hitung Kembalian
                                </div>
                            </div>
                        </button>
                        ...
                        <button
                            type="button"
                            :class="selectedMethod === 'edc' ? 'border-[#000000] bg-[#000000] text-white shadow-md' : 'border-neutral-300 bg-white text-neutral-800 hover:border-neutral-400 hover:bg-neutral-50'"
                            class="flex min-h-[72px] flex-col justify-between rounded-xl border p-3 text-left transition"
                            @click="selectedMethod = 'edc'"
                        >
                            <div class="flex items-center justify-between">
                                <CreditCard :class="selectedMethod === 'edc' ? 'text-[#beedc0]' : 'text-neutral-700'" class="h-5 w-5" />
                                <span :class="selectedMethod === 'edc' ? 'bg-white/20 text-white' : 'bg-neutral-100 text-neutral-700'" class="rounded px-1.5 py-0.5 text-[9px] font-semibold">
                                    Gesek/Dip
                                </span>
                            </div>
                            <div>
                                <div class="font-bold text-xs">Mesin EDC</div>
                                <div :class="selectedMethod === 'edc' ? 'text-neutral-300' : 'text-neutral-500'" class="text-[10px]">
                                    Kartu Debit / Kredit
                                </div>
                            </div>
                        </button>
                        ...
                        <button
                            type="button"
                            :class="selectedMethod === 'va' ? 'border-[#000000] bg-[#000000] text-white shadow-md' : 'border-neutral-300 bg-white text-neutral-800 hover:border-neutral-400 hover:bg-neutral-50'"
                            class="flex min-h-[72px] flex-col justify-between rounded-xl border p-3 text-left transition"
                            @click="selectedMethod = 'va'"
                        >
                            <div class="flex items-center justify-between">
                                <Building2 :class="selectedMethod === 'va' ? 'text-[#beedc0]' : 'text-neutral-700'" class="h-5 w-5" />
                                <span :class="selectedMethod === 'va' ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-800'" class="rounded px-1.5 py-0.5 text-[9px] font-semibold">
                                    VA Bank
                                </span>
                            </div>
                            <div>
                                <div class="font-bold text-xs">Virtual Account</div>
                                <div :class="selectedMethod === 'va' ? 'text-neutral-300' : 'text-neutral-500'" class="text-[10px]">
                                    BCA, BNI, Mandiri, BRI
                                </div>
                            </div>
                        </button>
                ...
                <div v-if="selectedMethod === 'qris'" class="space-y-4">
                    ...
                    <div v-if="!qrString" class="rounded-2xl border border-neutral-300 bg-white p-6 text-center shadow-xs">
                        <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#beedc0]/50 text-[#065f46]">
                            <QrCode class="h-6 w-6" />
                        </div>
                        <h4 class="font-bold text-sm text-[#000000]">Tampilkan Barcode QRIS Dinamis</h4>
                        <p class="mx-auto mt-1 max-w-sm text-xs text-neutral-600">
                            Sistem akan men-generate QRIS standar nasional secara otomatis sesuai total tagihan rawat jalan pasien.
                        </p>

                        <div class="mt-4 flex flex-wrap items-center justify-center gap-1.5 text-[10px] text-neutral-500">
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">BCA Mobile</span>
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">Livin Mandiri</span>
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">GoPay</span>
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">OVO</span>
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">ShopeePay</span>
                            <span class="rounded bg-neutral-100 px-2 py-0.5 font-semibold">DANA</span>
                        </div>
                    ...
                    <div v-else class="rounded-2xl border border-[#333333]/20 bg-white p-5 shadow-sm">
                        <!-- QRIS Header Motif -->
                        <div class="mb-3 flex items-center justify-between border-b border-neutral-200 pb-2.5">
                            <div class="flex items-center gap-2">
                                <span class="font-black text-sm tracking-wider text-rose-700">QRIS</span>
                                <span class="text-[10px] font-semibold text-neutral-400">|</span>
                                <span class="text-[10px] font-bold text-neutral-700">GPN INDONESIA</span>
                            </div>
                        ...
                        <div class="flex flex-col items-center justify-center p-2">
                            <div class="rounded-2xl border border-neutral-200 bg-white p-3 shadow-inner">
                                <canvas ref="qrCanvasRef" class="mx-auto block"></canvas>
                            </div>

                            <div class="mt-3 text-center">
                                <div class="font-['ivypresto-headline'] text-2xl font-black text-[#000000]">
                                    {{ formatRupiah(totalBill) }}
                                </div>
                                <p class="text-[11px] font-medium text-neutral-500">
                                    Minta pasien membuka aplikasi m-banking / e-wallet dan memindai barcode di atas
                                </p>
                            </div>
                        </div>

                        <!-- Live Polling Status Indicator -->
                        <div class="mt-4 flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50/70 px-4 py-2.5 text-xs text-amber-900">
                            <div class="flex items-center gap-2">
                                <Loader2 class="h-4 w-4 animate-spin text-amber-700" />
                                <span>Menunggu pembayaran pasien dari server...</span>
                            </div>
                            <span class="font-mono text-[11px] text-amber-800">Auto-check: 2.5s</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="selectedMethod === 'cash'" class="space-y-4">
                    <div class="rounded-2xl border border-neutral-300 bg-white p-5">
                        <label class="block text-xs font-semibold text-neutral-700">Nominal Uang Diterima dari Pasien (Rp)</label>
                        <input
                            v-model.number="cashReceived"
                            type="number"
                            step="1000"
                            class="mt-1.5 min-h-[44px] w-full rounded-xl border border-neutral-300 bg-white px-4 text-lg font-bold text-[#000000] focus:border-black focus:outline-none focus:ring-1 focus:ring-black"
                        />
                        ...
                        <div class="mt-4 rounded-xl border border-neutral-200 bg-[#edede2]/30 p-3.5 space-y-1 text-xs">
                            <div class="flex justify-between text-neutral-600">
                                <span>Total Tagihan:</span>
                                <span class="font-bold text-neutral-900">{{ formatRupiah(totalBill) }}</span>
                            </div>
                            <div class="flex justify-between text-neutral-600">
                                <span>Uang Diterima:</span>
                                <span class="font-bold text-neutral-900">{{ formatRupiah(cashReceived) }}</span>
                            </div>
                            <div class="mt-2 border-t border-neutral-300 pt-2 flex justify-between font-bold text-sm">
                                <span :class="cashReceived >= totalBill ? 'text-emerald-700' : 'text-rose-700'">
                                    {{ cashReceived >= totalBill ? 'Kembalian Pasien:' : 'Uang Masih Kurang:' }}
                                </span>
                                <span :class="cashReceived >= totalBill ? 'text-emerald-700' : 'text-rose-700'">
                ...
                <div v-else-if="selectedMethod === 'edc'" class="space-y-4">
                    <div class="rounded-2xl border border-neutral-300 bg-white p-5 space-y-3.5">
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700">Tipe Kartu</label>
                                <select
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700">Bank Penerbit EDC</label>
                                <select
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-neutral-700">
                                Nomor Approval Code / Reff Trace EDC <span class="text-rose-600">*</span>
                            </label>
                            <input
                                v-model="edcForm.approval_code"
                                type="text"
                                placeholder="Contoh: APPV-938201 / TRACE-8821"
                                class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-300 bg-white px-4 text-xs font-semibold text-neutral-900 focus:border-black focus:outline-none uppercase"
                            />
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-neutral-700">4 Digit Terakhir Kartu (Opsional)</label>
                            <input
                ...
                <div v-else-if="selectedMethod === 'va'" class="space-y-4">
                    <div class="rounded-2xl border border-neutral-300 bg-white p-5 text-center">
                        <div class="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-700">
                            <Building2 class="h-6 w-6" />
                        </div>
                        <h4 class="font-bold text-sm text-[#000000]">Virtual Account Bank & Checkout Online</h4>
                        <p class="mx-auto mt-1 max-w-sm text-xs text-neutral-600">
                            Buat tagihan online multi-bank (BCA, Mandiri, BNI, BRI, Permata) yang dapat dibayar melalui ATM/Internet Banking.
                        </p>

                        <div v-if="xenditInvoiceUrl" class="mt-4 rounded-xl border border-emerald-300 bg-emerald-50 p-4">
                            <div class="flex items-center justify-center gap-2 font-bold text-emerald-900 text-sm">
